import {
  ...
        try { await sendEmailVerification(signInRes.user); } catch (e) { console.warn('sendEmailVerification failed', e); }
        return {
          success: true,
          user: signInRes.user,
  ...
    },
    ...
    // Se ha simplificado esta función. Su única responsabilidad es autenticar
    // con Google y asegurar que el perfil de usuario exista en Firestore.
    // La lógica para decidir si es un "usuario nuevo" ahora vive en AuthContext.
    signInWithGoogle: async () => {
      try {
        const result = await signInWithPopup(auth, googleProvider);
  
        // Aseguramos que el perfil del usuario se cree en Firestore si no existe.
        await createProfileIfMissing(result.user);
        console.log('Google Auth - Profile ensured to exist for:', result.user.email);
